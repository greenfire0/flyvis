...
from itertools import product              # ← you'll need this later
from argparse import Namespace             # ← and this one too
import numpy as np
import pandas as pd
import torch
from datamate import Directory, root
from tqdm import tqdm
from .ft3d_utils import (
    download_sintel,
    load_sequence,
...
class MultiTaskFT3D(MultiTaskDataset):
    """Sintel dataset.
    ...
    def __init__(
        ...
        assert len(self.rendered) > 0 , "Dataset is empty"
        self.meta = sintel_meta(
            self.rendered, self.ft3d_path, n_frames, vertical_splits, "depth" in tasks
        )
...
class AugmentedFT3D(MultiTaskFT3D):
