...
def sintel_meta(
    ...
    lum_paths, sequence_indices, frames_per_scene = [], [], []
    sequence_index_to_splits: dict[int, np.ndarray] = {}

    # (A|B|C)/<sequence>/left png folders that survived your `RenderedFT3D` pass
    for i, left_dir in enumerate(
        sorted((sintel_path / "frames_cleanpass" / "TRAIN").rglob("left"))
    ):
        # Require ≥ n_frames **and** that at least one rendered key starts with this id
        if len(list(left_dir.iterdir())) >= n_frames and any(
            left_dir.parent.name in key for key in rendered
        ):
            lum_paths.append(left_dir)
            sequence_indices.append(i)
            frames_per_scene.append(len(list(left_dir.iterdir())))

        # map original sequence i → the indices created by vertical splitting
        sequence_index_to_splits[i] = vertical_splits * i + np.arange(vertical_splits)

    sequence_indices  = np.asarray(sequence_indices)
    frames_per_scene  = np.asarray(frames_per_scene)

    flow_paths  = [
        sintel_path / "optical_flow" / "TRAIN" / p.parent.parent.name / p.parent.name
        / "into_future" / "left"
        for p in lum_paths
    ]
    depth_paths = (
        [
            sintel_path / "depth" / "TRAIN" / p.parent.parent.name / p.parent.name / "left"
            for p in lum_paths
        ]
        if render_depth else None
    )
